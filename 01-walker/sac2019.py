...
class SACAgent:
    ...
    def load_checkpoint(self, policy_net, q_net):
        self.q_net1 = torch.load(f'{q_net}_q_net_1.pt', map_location=self.device)
        self.q_net2 = torch.load(f'{q_net}_q_net_2.pt', map_location=self.device)
        self.policy_net = torch.load(f'{policy_net}_policy.pt', map_location=self.device)